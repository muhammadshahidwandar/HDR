        ...
        private void GetFeatureVector(Image<Gray, byte> _image)
        {
            ...
            if (contours.Size >= 40)
            {
                ...
                else
                {
                    ...
                    ///<summary>
                    /// Storing feature points into csv file
                    /// </summary>

                    #region Storing feature vector into csv file

                    var numberofRows = _vectorsFeature[0].Count;
                    var rows = Enumerable.Range(0, numberofRows)  // For each Row
                    .Select(row => _vectorsFeature.Select(list => list[row]).ToList()).ToList();    // Get Row data from all columns
